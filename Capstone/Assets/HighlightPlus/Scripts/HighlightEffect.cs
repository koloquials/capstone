    ...
    public partial class HighlightEffect : MonoBehaviour {
        ...
        void DoOnRenderObject(Camera cam) {
            ...
            for (int k = 0; k < rmsCount; k++) {
                ...
                // Outline
                if (outlineIndependent && outline > 0 && outlineQuality != QualityLevel.Highest) {
                    for (int l = 0; l < mesh.subMeshCount; l++) {
                        if (outlineQuality == QualityLevel.High) {
                            for (int o = 0; o < offsets.Length; o++) {
                                ...
                                if (rms[k].skinnedMesh) {
                                    cbOutline.Clear();
                                    cbOutline.DrawRenderer(rms[k].renderer, fxMatOutline, l, 1);
                    ...
                    if (outline > 0 && outlineQuality != QualityLevel.Highest) {
                        ...
                        if (outlineQuality == QualityLevel.High) {
                            for (int o = 0; o < offsets.Length; o++) {
                                ...
                                if (rms[k].skinnedMesh) {
                                    cbOutline.Clear();
                                    cbOutline.DrawRenderer(rms[k].renderer, fxMatOutline, l, 0);
                                    Graphics.ExecuteCommandBuffer(cbOutline);
                                } else {
        ...
        void SmoothOutline(int rtWidth, int rtHeight) {
            ...
            for (int k = 0; k < bufferCount - 1; k += 2) {
                if (k == 0) {
                    cbSmoothBlend.Blit(sourceRT, mipOutlineBuffers[k + 1], fxMatBlurOutline, 0);
                } else {
                    cbSmoothBlend.Blit(mipOutlineBuffers[k], mipOutlineBuffers[k + 1], fxMatBlurOutline, 0);
                }
                cbSmoothBlend.Blit(mipOutlineBuffers[k + 1], mipOutlineBuffers[k], fxMatBlurOutline, 1);

                if (k < bufferCount - 2) {
                    cbSmoothBlend.Blit(mipOutlineBuffers[k], mipOutlineBuffers[k + 2], fxMatBlurOutline, 2);
                }
            }
        }
