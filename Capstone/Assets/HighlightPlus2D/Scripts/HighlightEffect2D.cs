...
namespace HighlightPlus2D {
    ...
    public class HighlightEffect2D : MonoBehaviour {
        ...
        public static Material fxMatSpriteMask, fxMatSpriteDepthWrite, fxMatSpriteSeeThrough, fxMatSpriteGlow;
        public static Material fxMatSpriteOutline, fxMatSpriteOverlay, fxMatSpriteShadow, fxMatSpriteShadow3D, fxMatSpriteClearStencil;
        ...
        void SetupMaterial() {
            ...
            for (int k = 0; k < rr.Length; k++) {
                ...
                rms[rmsCount].fxMatOverlay.renderQueue = overlayRenderQueue;
                ...
                HighlightGroup2D group = GetComponent<HighlightGroup2D>();
                if (group != null) {
                    renderQueueOffset += HighlightGroup2D.GROUP_OFFSET * group.groupNumber;
                }
                if (renderQueueOffset > 0) {
                    rms[rmsCount].fxMatMask.renderQueue += renderQueueOffset;
                    rms[rmsCount].fxMatOutline.renderQueue += renderQueueOffset;
                    rms[rmsCount].fxMatGlow.renderQueue += renderQueueOffset;
                    rms[rmsCount].fxMatSeeThrough.renderQueue += renderQueueOffset;
                    rms[rmsCount].fxMatOverlay.renderQueue += renderQueueOffset;
                }
                if (group == null && FindObjectOfType<HighlightGroup2D>() == null) {
                    if (glowOnTop) {
